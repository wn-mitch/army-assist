    ...
    switch (ability.ability_id) {
        case "000008338":
            return {
                ...
                name: `Feel No Pain`,
                description:
                    "Some warriors refuse to be laid low, even by what should be fatal wounds.Some models have âFeel No Pain x+' listed in their abilities. Each time a model with this ability suffers damage and so would lose a wound (including wounds lost due to mortal wounds), roll one D6: if the result is greater than or equal to the number denoted by âx', that wound is ignored and is not lost. If a model has more than one Feel No Pain ability, you can only use one of those abilities each time that model suffers damage and so would lose a wound.Feel No Pain x+: Each time this model would lose a wound, roll one D6: if the result equals or exceeds âx', that wound is not lost.",
            };
        case "000008346":
            return {
                ...ability,
...
const applyWeaponAndEnhancementOverrides = (
    ...
    const splitWeaponsAnd = (weapons: string[]): string[] => {
        ...
        return weapons.reduce((acc, curr) => {
            if (curr.includes(" and ")) {
                const splitItems = curr.split(" and ");
                const originalCount = counts[curr] || 0;
                delete counts[curr];
                splitItems.forEach((item) => {
                    counts[item] = (counts[item] || 0) + originalCount;
                });
    ...
    // Split weapons with " and " and update counts
    weapons = splitWeaponsAnd(weapons);

    switch (datasheet.name) {
        case "Ghazghkull Thraka":
            weapons = findAndReplace(weapons, "Gork’s Klaw", "Gork's Klaw");
            updateCounts("Gork’s Klaw", "Gork's Klaw");
            weapons = findAndReplace(weapons, "Mork’s Roar", "Mork's Roar");
            updateCounts("Mork’s Roar", "Mork's Roar");
            break;
        case "Banesword":
        case "Banehammer":
        case "Baneblade":
            ...
            updateCounts("Twin Heavy Bolters", "Twin heavy bolter");
            break;
        case "Flesh Hounds":
